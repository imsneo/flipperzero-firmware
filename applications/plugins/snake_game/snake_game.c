...
static void snake_game_render_callback(Canvas *
  const canvas, void * ctx) {
  ...
  if (snake_state -> state == GameStateGameOver) {
    ...
    char buffer[12];
    snprintf(buffer, sizeof(buffer), "Score: %u", snake_state -> len - 7 U);
...
static void snake_game_init_game(SnakeState *
  const snake_state) {
  Point p[] = {
    {
      8,
...
static Point snake_game_get_new_fruit(SnakeState
  const *
  const snake_state) {
...
static bool snake_game_collision_with_frame(Point
  const next_step) {
...
static bool
snake_game_collision_with_tail(SnakeState
  const *
  const snake_state, Point
  const next_step) {
  for (uint16_t i = 0; i < snake_state -> len; i++) {
    Point p = snake_state -> points[i];
...
static Direction snake_game_get_turn_snake(SnakeState
  const *
  const snake_state) {
...
static Point snake_game_get_next_step(SnakeState
  const *
  const snake_state) {
...
static void snake_game_move_snake(SnakeState *
  const snake_state, Point
  const next_step) {
...
static void
snake_game_process_game_step(SnakeState *
  const snake_state, NotificationApp * notification) {
  if (snake_state -> state == GameStateGameOver) {
    return;
  }
